...
const VerticalBarSecond = () => {
  return (
    <div className="w-1 min-h-full bg-cyan-600 mr-4 ml-8 text-cyan-600">.</div>
  );
};
...
export default function Page() {
  ...
  const AccordionCard = ({ itemNumber = "item-default", suivi }: any) => {
    return (
      <AccordionItem value={itemNumber} className="hover:bg-accent">
        <AccordionTrigger className="hover:no-underline">
          <div className="flex">
            <VerticalBarMain />
            <div>
              <div className=" ml-1 pl-3 text-left">{suivi.diagnostique}</div>
              <div className="font-light text-xs ml-1 pl-3 text-left">
          ...
          <div className="flex ">
            ...
            <div>
              ...
              <div className="font-light text-sm text-left">
                Dr. {getDocNameById(suivi.medecinProfileId)}
              </div>
            </div>
          </div>
        </AccordionContent>
  ...
  const NoDataComponent = () => {
    return (
      <div className="flex justify-center p-10">
        <Label>{"Aucun diagnostique n'a encore été enregistré"}</Label>
      </div>
    );
  };
