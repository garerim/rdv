...
import { EtatRendezVous, TypeRendezVous } from '../../components/checkout/rendezvous';
        ...
        if (session_id) {
            const fetchSession = async () => {
                try {
                    ...
                    if (session.metadata) {
                        const {
                            id, professionelId, patientId, rendezVousId,
                            prixAvantTVA, TVA, prixFinal, contenu, description,
                            startDate, duration, etat, typeRendezVous, prix, fichierJoint
                        } = session.metadata;
    ...
    return (
        <div className="flex items-center justify-center h-screen">
            <Card className="p-8 rounded-lg shadow-md max-w-md text-center">
                <h1 className="text-4xl font-bold mb-4 text-green-600">Rendez-vous créé avec succès</h1>
                <p className="text-gray-700 mb-6">Votre rendez-vous prévu le {formatDate(rendezVous.startDate)} à {formatHour(rendezVous.startDate)} est confirmé.</p>
                <Button onClick={() => window.location.href = '/rendez_vous'} variant={"default"}>J'ai compris</Button>
            </Card>
        </div>
    );
};
